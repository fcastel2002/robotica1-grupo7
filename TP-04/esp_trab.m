...
function esp_trab()
    ...
    % Sobrescribimos R1.base para no trasladar todos los gráficos.
    R1.base = eye(4);
    R1.tool = eye(4);
    % --- Parámetros de la simulación ---
    pasos = 200; % Puntos a calcular por cada articulación.
    ...
    contorno_xz = zeros(length(q2_range),2);
    ...
    contorno_ext_xy = zeros(length(q1_range), 2);
    
    % Simplificación para alcance máximo y mínimo
    q_alcance = [0, R1.qlim(2,2), R1.qlim(3,1), q_muneca];
    i = 0;
    for q1 = q1_range
        i =i+1;
        q_alcance(1) = q1;
        T_ext = R1.fkine(q_alcance);
        contorno_ext_xy(i, :) = [T_ext.t(1), T_ext.t(2)];
    end

    figure('Name', 'Workspace Contorno - Vista Superior (Plano XY)', 'NumberTitle', 'off');
    hold on;
    plot(contorno_ext_xy(:,1), contorno_ext_xy(:,2), 'r-');
    axis([-1.2 1.2 -1.2 1.2]);
    title('Contorno del Espacio de Trabajo (Plano XY)');
    xlabel('Alcance en X (m)'); ylabel('Alcance en Y (m)');
    legend('Contorno Exterior');
    grid on; axis equal;

    
    % 1. Cota para el radio exterior
    line([0,contorno_ext_xy(1,1)],[0,contorno_ext_xy(1,2)], 'Color','r');
    line([0,contorno_ext_xy(end,1)],[0,contorno_ext_xy(end,2)],'Color','r');
    punto_ext = contorno_ext_xy(50,:); % Tomamos el primer punto del contorno
