...
plist_R2 = {
    ...
    %   1.2. Trayectoria (Inicio)
    struct('pose',[-0.2,  0.5, 0.3, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),

    % PUNTOS INTERMEDIOS PARA EVITAR SINGULARIDAD (1.2 -> 1.3) - OPCIÓN 2
    % Estrategia: Movimiento en arco "por detrás" cambiando Y para evitar singularidad
    
    % 1.2a: Subir y desplazarse hacia atrás (Y negativo)
    struct('pose',[-0.2,  0.4, 0.45, 0.05, -pi/2+0.1, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.2b: Continuar subiendo con desplazamiento
    struct('pose',[-0.2,  0.35, 0.55, 0.08, -pi/2+0.15, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.2c: Subir más y volver hacia la posición original en Y
    struct('pose',[-0.2,  0.4, 0.65, 0.05, -pi/2+0.1, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.2d: Volver a Y=0.5 mientras subimos
    struct('pose',[-0.2,  0.48, 0.68, 0.02, -pi/2+0.05, -pi/2+0.01],'tipo','cartesiana'),

    %   1.3. (NUEVO) Mover Z-UP (Retirada)
    %      (Misma X,Y, misma Orientación, solo Z más alto)
    struct('pose',[-0.2,  0.5, 0.7, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),

    % SOLUCIÓN: Trayectoria curva dentro del alcance del robot
    % Estrategia: Crear una curva moderada que suba un poco Z y aumente Y moderadamente
    % manteniendo todos los puntos dentro del alcance del robot. PITCH constante.
    
    % 1.3a: Empezar curva: subir Z moderadamente, aumentar Y poco
    struct('pose',[-0.2,  0.52, 0.72, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.3b: Continuar curva: avanzar en X, Y un poco más alto, Z un poco más alto
    struct('pose',[-0.1,  0.54, 0.74, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.3c: Punto medio de la curva: X intermedio, Y máximo moderado, Z máximo moderado
    struct('pose',[0.05,  0.55, 0.75, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.3d: Continuar curva: avanzar más en X, empezar a reducir Y, mantener Z moderado
    struct('pose',[0.15,  0.54, 0.74, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),
    
    % 1.3e: Casi en posición final: X casi final, Y reduciendo, Z bajando gradualmente
    struct('pose',[0.22,  0.52, 0.72, 0.01, -pi/2+0.01, -pi/2+0.01],'tipo','cartesiana'),

    %   1.4. (NUEVO) Mover XY-ACROSS (Viaje)
    %      (Nueva X,Y, pero Z se mantiene ALTO)
    %      PITCH se mantiene constante para no volcar el café
    struct('pose',[0.25,  0.5, 0.7, 0.01, -pi/2+0.01,  -pi/2+0.01],'tipo','cartesiana'),

    %   1.5. (NUEVO) Mover Z-DOWN (Aproximación)
    %      (X,Y final, Z final, Orientación final)
...
% groups_R2 actualizado con SOLUCIÓN CURVA (bajar Z y aumentar Y moderadamente):
% - OPCIÓN 2: 4 puntos intermedios entre 1.2 (índice 3) y 1.3 (índice 8)
...
% Modo dinámico: usa la pose actual del robot después del vuelco
fig = gcf;
...
uicontrol('Parent',fig,'Style','pushbutton','Units','normalized', ...
    ...
    'FontSize',11, ...
    'Callback', @(~,~) arte_latte_corazon(R1));
